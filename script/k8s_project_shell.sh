...
if [ -z "$pod_name" ]; then
  echo "Usage: make k8s-shell <pod-name> [namespace]"
  echo "Example: make k8s-shell cloudwatch-agent-g58hs"
  echo "Example: make k8s-shell cloudwatch-agent-g58hs kube-system"
  exit 1
fi
...
if [ -z "$namespace" ]; then
  ...
  if [ "$match_count" -gt 1 ]; then
    ...
    echo "Specify a namespace explicitly: make k8s-shell $pod_name <namespace>"
    exit 1
  fi
...
printf "\n${div}\n Opening shell to pod '%s' in namespace '%s' on cluster '%s'\n${div}\n\n" "$pod_name" "$namespace" "$cluster_selection"
printf "kubectl exec -it -n %s %s --kubeconfig=%s -- /bin/bash\n\n" "$namespace" "$pod_name" "$KUBECONFIG"

kubectl exec -it -n "$namespace" "$pod_name" --kubeconfig="$KUBECONFIG" -- /bin/bash
